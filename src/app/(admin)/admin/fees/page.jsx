...
export default function FeesPage() {
  ...
  const [formData, setFormData] = useState({ studentId: '', amount: '', paymentDate: '', paymentMethod: 'Cash', status: 'Paid', remarks: '' });
  ...
  const handleOpenModal = (record = null) => {
    if (record) {
      ...
      setIsEditing(false);
      setEditId(null);
      setFormData({ studentId: '', amount: '', paymentDate: new Date().toISOString().split('T')[0], paymentMethod: 'Cash', status: 'Paid', remarks: '' });
    }
    setShowModal(true);
  };
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-6">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h1 className="text-2xl font-bold font-poppins text-slate-900">Manage Student Fees</h1>
          <p className="text-slate-500 text-sm mt-1">Track payments, issue receipts, and monitor dues.</p>
        </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/50 backdrop-blur-sm">
          <div className="bg-white rounded-xl shadow-xl w-full max-w-md overflow-hidden animate-in fade-in zoom-in-95 duration-200">
            <div className="px-6 py-4 border-b border-slate-200 flex justify-between items-center bg-slate-50">
              <h2 className="text-lg font-bold text-slate-900">{isEditing ? 'Edit Fee Record' : 'Log New Payment'}</h2>
